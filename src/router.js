...
export default (props) => {
  ...
  const { getDynamicRouters, staticRouters, erroWrap, pageError = {}, routerUpdateInterval = 15 * 60 * 1000 } = routerConfig
  const { dynamicRouters, setDynamicRouters } = useState([])
  const NoMatch = pageError.noMatch || defaultNoMatch
  const ErrorWrap = (typeof erroWrap === 'string' ? loadable(() => import(`../execption/${erroWrap}`)) : erroWrap) || ErrorBoundary
          ...
          <Switch>
            {
              staticRouters.map((frame, i) => {
                const {pageError = {}} = frame
                const Layout = typeof frame.layout === 'string' ? loadable(() => import(`../layouts/${frame.layout}`)) : frame.layout
                const NoMatchComponent = typeof pageError.noMatch === 'string' ? loadable(() => import(`./${pageError.noMatch}`)) : pageError.noMatch
                return (
                    ...
                    <Layout key={`static-router-${frame.title || 'router'}-${i}`}>
                      <ErrorWrap>
                        <Switch>
                          {
                            frame.routers.map((r) => {
                              const component = typeof r.component === 'string' ? loadable(() => import(`../components/${r.component}`)) : r.component
                              return (<Route
                                exact
                                key={r.title}
                                component={component}
